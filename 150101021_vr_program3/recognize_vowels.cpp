...
int main(int argc, char const *argv[]) {
	...
	for (int v = 0; v < 5; v++) {
		for (int fileNo = 0; fileNo < 10; fileNo++) {
			char TEST_VOWEL = VOWELS[v];

			std::ostringstream filename;
			filename.str(std::string());
			bool TEST_TRIMMED = TEST_TRIMMED_ALREADY;
			if (argc > 1) {
				filename << argv[1];
			}
			else {
				//filename << CURR_DIR <<"_";
				TEST_TRIMMED = 1;
				filename << CURR_DIR << "output_clips/150101021_" << TEST_VOWEL << "_"<< fileNo<<".txt";
				// filename << CURR_DIR <<"output_clips/150101021_e_2.txt";
				// filename << CURR_DIR <<"output_clips/150101021_e_3.txt";
				// filename << CURR_DIR <<"output_clips/150101021_e_8.txt";
				// filename << CURR_DIR <<"output_clips/150101021_i_8.txt";
				// filename << CURR_DIR <<"output_clips/150101021_e_9.txt";
				// filename << CURR_DIR <<"output_clips/150101021_o_1.txt";
			}
			ifstream INPUT_FS;
			...
			for (int i = 0; i < N_SAMPLES; ++i) {
				// sscanf_s(&lines[i + OFFSET][0], "%lf", &x);
			...
			*/
			...
			for (int i = 0; i < NUM_CLIPS; ++i) {
				...
				if (frameWiseCis.size() < FIRST_N_FRAMES) {
					cout << "ERROR: Insufficient No Of Frames: " << frameWiseCis.size() << "/" << FIRST_N_FRAMES << endl;
					continue;
				}
				/// Print distances
